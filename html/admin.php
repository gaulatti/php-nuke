...
if ((isset($aid)) && (isset($pwd)) && (isset($op)) && ($op == "login")) {
	...
	if(!empty($aid) AND !empty($pwd)) {
		...
		list($rpwd, $admlanguage) = $result->fetch_row();
...
if(isset($admin) && !empty($admin)) {
	...
	if (!$result2) {
		...
		list($rname, $rpwd) = $result2->fetch_row();
...
function adminMain() {
	...
	list($aidname) = $result2->fetch_row();
	$radminarticle = 0;
	while (list($admins) = $result->fetch_row()) {
	...
	list($userCount) = $db->sql_query("SELECT COUNT(user_id) AS userCount from ".$user_prefix."_users WHERE user_regdate LIKE '$curDate2'")->fetch_row();
	list($userCount2) = $db->sql_query("SELECT COUNT(user_id) AS userCount FROM ".$user_prefix."_users WHERE user_regdate LIKE '$curDateP'")->fetch_row();
	...
	if (is_active("News")) {
		...
		while (list($anid, $aid, $listtitle, $time, $alanguage) = $result5->fetch_row()) {
			$anid = intval($anid);
...
if($admintest) {
	switch($op) {
		...
		while (list($mod_title) = $result->fetch_row()) {
